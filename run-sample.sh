...
ENABLE_PROFILING=${enableProfiling} ./dev/instrument.sh OTEL_DOTNET_AUTO_ENABLED_INSTRUMENTATIONS="AspNet,SqlClient" OTEL_SERVICE_NAME="aspnet-server" OTEL_TRACES_EXPORTER=${exporter} ASPNETCORE_URLS="http://127.0.0.1:8080/" dotnet ./samples/Samples.AspNetCoreMvc31/bin/${configuration}/${aspNetAppTargetFramework}/Samples.AspNetCoreMvc31.dll &
unset OTEL_DOTNET_AUTO_INSTRUMENTATION_PLUGINS
./dev/wait-local-port.sh 8080

# instrument and run HTTP client app
ENABLE_PROFILING=${enableProfiling} ./dev/instrument.sh OTEL_DOTNET_AUTO_ENABLED_INSTRUMENTATIONS="HttpClient" OTEL_SERVICE_NAME="http-client" OTEL_TRACES_EXPORTER=${exporter} OTEL_DOTNET_AUTO_LOAD_AT_STARTUP=${sampleAppInjectSDK} dotnet ./samples/${sampleApp}/bin/$configuration/${sampleAppTargetFramework}/${sampleApp}.dll
