    ...
    internal class DiagnosticSourceEventSource : EventSource
    {
        ...
        internal class FilterAndTransform
        {
            ...
            internal static bool IsActivitySourceEntry(string filterAndPayloadSpec, int startIdx, int endIdx) =>
                            filterAndPayloadSpec.Substring(startIdx, endIdx - startIdx).StartsWith(c_ActivitySourcePrefix, StringComparison.Ordinal);
            ...
            internal static void AddNewActivitySourceTransform(string filterAndPayloadSpec, int startIdx, int endIdx, DiagnosticSourceEventSource eventSource)
            {
                ...
                string entry = filterAndPayloadSpec.Substring(
                ...
                if (eventNameIndex >= 0)
                {
                    activitySourceName = entry.Substring(0, eventNameIndex).Trim();

                    string suffixPart = entry.Substring(eventNameIndex + 1, entry.Length - eventNameIndex - 1).Trim();
                    ...
                    if (samplingResultIndex >= 0)
                    {
                        ...
                        eventName = suffixPart.Substring(0, samplingResultIndex).Trim();
                        suffixPart = suffixPart.Substring(samplingResultIndex + 1, suffixPart.Length - samplingResultIndex - 1).Trim();

                        if (suffixPart.Length > 0)
                        {
                            if (suffixPart.Equals("Propagate", StringComparison.OrdinalIgnoreCase))
                            {
                                samplingResult = ActivitySamplingResult.PropagationData;
                            }
                            else if (suffixPart.Equals("Record", StringComparison.OrdinalIgnoreCase))
                            {
                    ...
                    if (eventName.Length > 0)
                    {
                        ...
                        else if (eventName.Equals("Stop", StringComparison.OrdinalIgnoreCase))
                        {
                ...
                if (plusSignIndex >= 0)
                {
                    activityName = activitySourceName.Substring(plusSignIndex + 1).Trim();
                    activitySourceName = activitySourceName.Substring(0, plusSignIndex).Trim();
                }
