...
public class InstrumentationDefinitionsGenerator : IIncrementalGenerator
{
    ...
    private const int IntegrationKindInterface = 2;
    ...
    private static void Generate(in ImmutableArray<IntegrationToGenerate?> instrumentationClasses, SourceProductionContext context)
    {
        ...
        var interfaceIntegrations = GenerateInstrumentationDefinitionsPartialClass(instrumentationClasses, IntegrationKindInterface);
        context.AddSource("InstrumentationDefinitions.Interface.g.cs", SourceText.From(interfaceIntegrations, Encoding.UTF8));
    }
        ...
        ImmutableArray<IntegrationToGenerate?> integrationClasses, int integrationKind)
    {
        ...
        var generatedMethodName = integrationKind switch
        {
            IntegrationKindDirect => "GetDefinitionsArray",
            IntegrationKindDerived => "GetDerivedDefinitionsArray",
            IntegrationKindInterface => "GetInterfaceDefinitionsArray",
            _ => throw new ArgumentOutOfRangeException(nameof(integrationKind)),
        };
