...
namespace OpenTelemetry.AutoInstrumentation.ClrProfiler.Emit
{
    internal static class ModuleLookup
    {
        ...
        private static readonly ILogger Log = ConsoleLogger.Create(typeof(ModuleLookup));
        ...
        public static Module Get(Guid moduleVersionId)
        {
            ...
            if (_failures >= MaxFailures)
            {
                ...
                {
                    Log.Warning("OpenTelemetry is unable to continue attempting module lookups for this AppDomain. Falling back to legacy method lookups.");
                }
