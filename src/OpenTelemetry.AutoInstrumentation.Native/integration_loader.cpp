...
void LoadIntegrationsFromEnvironment(std::vector<IntegrationMethod>&     integrationMethods,
...
void LoadIntegrationsFromFile(const WSTRING&                      file_path,
...
void LoadIntegrationsFromStream(std::istream&                       stream,
                                std::vector<IntegrationMethod>&     integrationMethods,
                                const LoadIntegrationConfiguration& configuration)
{
    try
...
void MethodReplacementFromJson(const json::value_type&                src,
                               const WSTRING&                         integrationName,
                               std::unordered_set<IntegrationMethod>& integrationMethods)
{
    if (src.is_object())
    {
        const MethodReference wrapper = MethodReferenceFromJson(src.value("wrapper", json::object()), false, true);
        const MethodReference target  = MethodReferenceFromJson(src.value("target", json::object()), true, false);
