    ...
    : profilerApi_(profilerApi)
{
    ...
    safetyProbe_ =
        std::make_unique<StackSafetyProbe>(profilerApi, queue, kClrSuspensionProbeFlags, CaptureOptions{}.probeTimeout);
#else
    (void)queue;
#endif
}
...
void ClrRuntimeSuspensionPolicy::EndBatch() noexcept
{
    ...
    if (FAILED(hr))
    {
        trace::Logger::Error("[ClrRuntimeSuspensionPolicy] ResumeRuntime failed. HRESULT=", trace::HResultStr(hr));
    }
}
...
void OsThreadSuspensionPolicy::OnThreadDestroyed(ThreadID threadId)
{
    ...
    trace::Logger::Info("[OsThreadSuspensionPolicy] Canary thread destroyed - ManagedID=", threadId,
    ...
    for (const auto& [managedId, name] : threadNames_)
    {
        ...
        trace::Logger::Info("[OsThreadSuspensionPolicy] New canary thread designated - ManagedID=", managedId,
                            ", NativeID=", osThreadIt->second, ", Name=", name);
        captureCondVar_.notify_all();
        break;
    }
}
...
void OsThreadSuspensionPolicy::OnThreadNameChanged(ThreadID threadId, ULONG cchName, WCHAR name[])
{
    ...
    trace::Logger::Debug("[OsThreadSuspensionPolicy] ThreadNameChanged - ManagedID=", threadId, ", Name=", threadName);
    ...
    {
        trace::Logger::Debug(
            "[OsThreadSuspensionPolicy] Canary name matched but OS thread not yet assigned - ManagedID=", threadId);
        return;
    }
...
CanaryThreadInfo OsThreadSuspensionPolicy::WaitForCanaryThread(std::chrono::milliseconds timeout)
{
    ...
    if (!result)
    {
        trace::Logger::Warn("[OsThreadSuspensionPolicy] Canary thread wait timed out after ", timeout.count(), "ms");
        return canary;
    }

    canary = canaryThread_;

    trace::Logger::Debug("[OsThreadSuspensionPolicy] Canary thread ready - ManagedID=", canary.managedId,
