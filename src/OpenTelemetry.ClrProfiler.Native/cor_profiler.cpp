  ...
  if (is_instrumentation_assembly || debug_logging_enabled) {
    ...
    if (is_instrumentation_assembly) {
      ...
      if (assembly_version == ToWSTRING(PROFILER_VERSION)) {
        Info("AssemblyLoadFinished: AssemblyName=", assembly_info.name, " AssemblyVersion=", assembly_metadata.version.str(), " matched profiler version v", PROFILER_VERSION);
        ...
        if (runtime_information_.is_desktop() && corlib_module_loaded) {
          ...
          if (assembly_info.app_domain_id == corlib_app_domain_id) {
            Info("AssemblyLoadFinished: AssemblyName=", assembly_info.name, " was loaded domain-neutral");
            managed_profiler_loaded_domain_neutral = true;
          } else {
            Info("AssemblyLoadFinished: AssemblyName=", assembly_info.name, " was not loaded domain-neutral");
          }
        }
      } else {
        Warn("AssemblyLoadFinished: AssemblyName=", assembly_info.name, " AssemblyVersion=", assembly_metadata.version.str(), " did not match profiler version v", PROFILER_VERSION);
      }
    }
  }
...
HRESULT STDMETHODCALLTYPE CorProfiler::ModuleLoadFinished(ModuleID module_id,
  ...
  if (module_info.assembly.name == WStr("OpenTelemetry.ClrProfiler.Managed.Loader")) {
    Info("ModuleLoadFinished: OpenTelemetry.ClrProfiler.Managed.Loader loaded into AppDomain ", app_domain_id, " ", module_info.assembly.app_domain_name);
...
HRESULT STDMETHODCALLTYPE CorProfiler::JITCompilationStarted(
  ...
  // OpenTelemetry.ClrProfiler.Managed.dll and its dependencies on-disk since it
...
Debug("GenerateVoidILStartupMethod: Linux: Setting the PInvoke native profiler library path to ", native_profiler_file);
  ...
  LPCWSTR load_helper_str =
      L"OpenTelemetry.ClrProfiler.Managed.Loader.Startup";
  auto load_helper_str_size = wcslen(load_helper_str);
#else
  char16_t load_helper_str[] =
      u"OpenTelemetry.ClrProfiler.Managed.Loader.Startup";
