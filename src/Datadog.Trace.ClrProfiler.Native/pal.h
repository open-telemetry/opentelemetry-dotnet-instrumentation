...
inline WSTRING DatadogLogFilePath(const std::string& file_name_suffix)
{
    ...
    return ToWSTRING(program_data + R"(\OpenTelemetry .NET AutoInstrumentation\logs\dotnet-tracer-native)" + file_name_suffix + ".log");
#else
    return ToWSTRING("/var/log/opentelemetry/dotnet/dotnet-tracer-native" + file_name_suffix + ".log");
#endif
}
