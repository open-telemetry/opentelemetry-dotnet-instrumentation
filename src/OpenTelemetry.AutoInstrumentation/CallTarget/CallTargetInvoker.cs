...
public static class CallTargetInvoker
{
    ...
    /// <returns>CallTarget return structure</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static CallTargetReturn EndAsyncMethod<TIntegration, TTarget>(TTarget instance, Exception exception, in CallTargetState state)
    {
        if (IntegrationOptions<TIntegration, TTarget>.IsIntegrationEnabled)
        {
            RuntimeAsyncEndMethodHandler<TIntegration, TTarget, object>.Invoke(instance, null, exception, in state);
        }
    ...
    /// <typeparam name="TIntegration">Integration type</typeparam>
    /// <typeparam name="TTarget">Target type</typeparam>
    /// <typeparam name="TReturn">Logical async result type</typeparam>
    /// <param name="instance">Instance value</param>
    /// <param name="returnValue">Logical async result value</param>
    /// <param name="exception">Exception value</param>
    /// <param name="state">CallTarget state</param>
    /// <returns>CallTarget return structure</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static CallTargetReturn<TReturn?> EndAsyncMethod<TIntegration, TTarget, TReturn>(TTarget instance, TReturn returnValue, Exception exception, in CallTargetState state)
    {
        TReturn? result = returnValue;
        if (IntegrationOptions<TIntegration, TTarget>.IsIntegrationEnabled)
        {
            result = RuntimeAsyncEndMethodHandler<TIntegration, TTarget, TReturn>.Invoke(instance, returnValue, exception, in state);
        }
