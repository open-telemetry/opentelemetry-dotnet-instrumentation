//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by the LibraryVersionsGenerator tool. To safely
//     modify this file, edit PackageVersionDefinitions.cs and
//     re-run the LibraryVersionsGenerator project in Visual Studio.
// 
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated. 
// </auto-generated>
//------------------------------------------------------------------------------

using Models;

public static partial class LibraryVersion
{
    public static IReadOnlyDictionary<string, IReadOnlyCollection<PackageBuildInfo>> Versions = new Dictionary<string, IReadOnlyCollection<PackageBuildInfo>>
    {
        {
            "TestApplication.Azure",
            [
                new("12.22.2"),
                new("12.26.0"),
            ]
        },
        {
            "TestApplication.Elasticsearch",
            [
                new("8.15.10"),
                new("9.5.2"),
            ]
        },
        {
            "TestApplication.EntityFrameworkCore",
            [
                new("6.0.36"),
                new("8.0.23"),
                new("9.0.12"),
                new("10.0.0", supportedFrameworks: [ "net11.0", "net10.0" ]),
                new("10.0.12", supportedFrameworks: [ "net11.0", "net10.0" ]),
            ]
        },
        {
            "TestApplication.EntityFrameworkCore.Pomelo.MySql",
            [
                new("6.0.3"),
                new("7.0.0"),
                new("8.0.3"),
                new("9.0.0"),
            ]
        },
        {
            "TestApplication.GraphQL",
            [
                new("7.5.0", additionalMetaData: new() { { "GraphQLMicrosoftDI", "7.5.0" }, { "GraphQLServerTransportsAspNetCore", "7.5.0" }, { "GraphQLServerUIGraphiQL", "7.5.0" } }),
                new("8.0.2", additionalMetaData: new() { { "GraphQLMicrosoftDI", "8.0.2" }, { "GraphQLServerTransportsAspNetCore", "8.0.2" }, { "GraphQLServerUIGraphiQL", "8.0.2" } }),
                new("8.8.5", additionalMetaData: new() { { "GraphQLMicrosoftDI", "8.8.5" }, { "GraphQLServerTransportsAspNetCore", "8.3.3" }, { "GraphQLServerUIGraphiQL", "8.3.3" } }),
            ]
        },
        {
            "TestApplication.GrpcNetClient",
            [
                new("2.52.0"),
                new("2.83.0"),
            ]
        },
        {
            "TestApplication.Log4NetBridge",
            [
                new("3.3.0"),
                new("3.4.0"),
            ]
        },
        {
            "TestApplication.NLogBridge",
            [
                new("5.0.0", additionalMetaData: new() { { "NLogExtensionsLogging", "5.0.0" } }),
                new("5.3.4", additionalMetaData: new() { { "NLogExtensionsLogging", "5.3.15" } }),
                new("6.0.0", additionalMetaData: new() { { "NLogExtensionsLogging", "6.0.0" } }),
                new("6.2.0", additionalMetaData: new() { { "NLogExtensionsLogging", "6.2.0" } }),
            ]
        },
        {
            "TestApplication.MassTransit",
            [
                new("8.3.0"),
                new("8.5.10"),
            ]
        },
        {
            "TestApplication.SqlClient.Microsoft",
            [
                new("5.2.3"),
                new("6.1.5"),
                new("7.0.3"),
            ]
        },
        {
            "TestApplication.SqlClient.System",
            [
                new("4.8.6"),
                new("4.9.1"),
            ]
        },
        {
            "TestApplication.MongoDB",
            [
                new("2.19.0", supportedFrameworks: [ "net11.0", "net10.0", "net462" ]),
                new("2.30.0", supportedFrameworks: [ "net11.0", "net10.0", "net462" ]),
                new("3.0.0", supportedFrameworks: [ "net11.0", "net10.0", "net472" ]),
                new("3.5.0", supportedFrameworks: [ "net11.0", "net10.0", "net472" ]),
                new("3.7.0", supportedFrameworks: [ "net11.0", "net10.0", "net472" ]),
                new("3.11.1", supportedFrameworks: [ "net11.0", "net10.0", "net472" ]),
            ]
        },
        {
            "TestApplication.MySqlConnector",
            [
                new("2.0.0"),
                new("2.6.2"),
            ]
        },
        {
            "TestApplication.MySqlData",
            [
                new("9.0.0"),
                new("26.7.0"),
            ]
        },
        {
            "TestApplication.Npgsql",
            [
                new("8.0.9"),
                new("9.0.4", supportedFrameworks: [ "net11.0", "net10.0" ]),
                new("10.0.3", supportedFrameworks: [ "net11.0", "net10.0" ]),
            ]
        },
        {
            "TestApplication.NServiceBus",
            [
                new("8.2.6"),
                new("9.2.10", supportedFrameworks: [ "net11.0", "net10.0" ]),
                new("10.1.3", supportedFrameworks: [ "net11.0", "net10.0" ]),
                new("10.2.8", supportedFrameworks: [ "net11.0", "net10.0" ]),
            ]
        },
        {
            "TestApplication.OracleMda.NetFramework",
            [
                new("23.5.1", supportedFrameworks: [ "net472" ]),
                new("23.26.301", supportedFrameworks: [ "net472" ]),
            ]
        },
        {
            "TestApplication.OracleMda.Core",
            [
                new("23.5.1"),
                new("23.26.301"),
            ]
        },
        {
            "TestApplication.Quartz",
            [
                new("3.6.0"),
                new("3.20.0"),
            ]
        },
        {
            "TestApplication.RabbitMq",
            [
                new("5.1.2"),
                new("6.8.1"),
                new("7.0.0"),
                new("7.2.2"),
            ]
        },
        {
            "TestApplication.Sqlite.Microsoft",
            [
                new("8.0.0"),
                new("9.0.0"),
                new("10.0.0"),
                new("10.0.12"),
            ]
        },
        {
            "TestApplication.StackExchangeRedis",
            [
                new("2.6.122"),
                new("2.12.14"),
                new("3.0.25", supportedFrameworks: [ "net11.0", "net10.0" ]),
                new("3.1.0", supportedFrameworks: [ "net11.0", "net10.0" ]),
                new("3.1.31", supportedFrameworks: [ "net11.0", "net10.0" ]),
            ]
        },
        {
            "TestApplication.Wcf.Client.DotNet",
            [
                new("4.10.2"),
                new("6.2.0"),
                new("8.1.2"),
                new("10.0.652802", supportedFrameworks: [ "net11.0", "net10.0" ]),
            ]
        },
        {
            "TestApplication.Kafka",
            [
                new("1.8.2"),
                new("1.9.2"),
                new("2.15.0"),
            ]
        },
        {
            "TestApplication.AssemblyRedirection",
            [
                new("6.0.0", supportedFrameworks: [ "net462" ]),
                new("10.0.12", supportedFrameworks: [ "net462", "net11.0", "net10.0" ]),
                new("11.0.0-preview.5.26302.115", supportedFrameworks: [ "net462", "net11.0", "net10.0" ]),
            ]
        },
        {
            "TestApplication.Wcf.Core",
            [
                new("1.8.1", supportedFrameworks: [ "net11.0", "net10.0" ]),
                new("1.9.1", supportedFrameworks: [ "net11.0", "net10.0" ]),
            ]
        },
    };
}
