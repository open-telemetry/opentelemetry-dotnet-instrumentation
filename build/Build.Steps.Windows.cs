...
partial class Build
{
    ...
    Target GenerateNetFxTransientDependencies => _ => _
        ...
        .Executes(() =>
        {
            string GetLabel([CanBeNull] string framework) =>
                framework != null
                    ? $"Transient dependencies auto-generated by {nameof(GenerateNetFxTransientDependencies)} for {framework}"
                    : $"Transient dependencies auto-generated by {nameof(GenerateNetFxTransientDependencies)}";

            (string PackageName, string PackageVersion) GetPackageProperties(ProjectElement item) =>
                item is ProjectItemElement { ElementName: "PackageVersion" } pkgRef
                    ? (pkgRef.Include,
                        pkgRef.Metadata.FirstOrDefault(it => it.Name == "Version")?.Value)
                    : (null, null);

            void AddPackageReference(ProjectItemGroupElement items, string packageName, string packageVersion)
            {
            ...
            var packages = Solution.Directory / "src" / Projects.AutoInstrumentationNetFxAssemblies /
            ...
            {
                Log.Error(
                    $"Project {targetProject.Name} does not specify any target frameworks. Skipping {nameof(GenerateNetFxTransientDependencies)}.");
                return;
            }
            ...
            void CleanUnusedDependencies([CanBeNull] string framework)
            {
                var label = GetLabel(framework);
                var versionGroup = packagesProject.Xml.ItemGroups.First(x => x.Label == label);
            ...
            {
                CleanUnusedDependencies(framework);
            }

            CleanUnusedDependencies(null);
